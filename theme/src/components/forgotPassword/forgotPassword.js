...
class ForgotPassword extends React.Component {
	...
	render() {
		const { handleSubmit, customerproperties } = this.props;
			...
			<div className="login-container">
				<form onSubmit={handleSubmit} className="login-form">
					<div className="login-section">
						<h1 className={titleClassName}>{text.forgot_password}</h1>
						<p className={titleClassName}>{text.forgot_password_subtitle}</p>
						{this.props.customerproperties !== undefined &&
						this.props.customerproperties.loggedin_failed ? (
							<p>{text.login_failed}</p>
						) : (
							''
						)}
